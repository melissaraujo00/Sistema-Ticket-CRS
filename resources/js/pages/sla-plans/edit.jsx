...
export default function Edit({ slaPlan }) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        name: slaPlan.name || "",
        grace_time_hours: slaPlan.grace_time_hours ?? "",
        working_hours: slaPlan.working_hours ?? true,
    });
    ...
    // Breadcrumbs adaptados a la edición
    const breadcrumbs = [
        { title: "Dashboard", href: "/dashboard" },
        { title: "Planes SLA", href: "/sla-plans" },
        { title: `Editar: ${slaPlan.name}`, href: `/sla-plans/${slaPlan.id}/edit` },
    ];

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Editar Plan SLA - ${slaPlan.name}`} />
            <form
                onSubmit={submit}
            ...
            >
                ...
                <div className="p-6 dark:bg-zinc-900/20 border-t border-zinc-100 dark:border-zinc-800 mt-auto">
                    <div className="flex items-center justify-end gap-4">
                        ...
                        >
                            {processing ? <Loader2 className="h-4 w-4 animate-spin" /> : "Actualizar Plan SLA"}
                        </Button>
                    </div>
                </div>
            </form>
