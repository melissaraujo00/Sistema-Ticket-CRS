...
import { Head, Link, usePage } from '@inertiajs/react'; // <-- Agrega usePage
...
export default function Trashed({ areas = [] }) {
    ...
    // 1. Extraemos los mensajes del backend
    const { flash } = usePage().props;

    // 2. Escuchamos los mensajes de éxito/error y disparamos el toast
    useEffect(() => {
    ...
    const columns = [
        {
            header: 'Nombre',
            render: (area) => <span className="font-semibold">{area.name}</span>,
        },
        {
            header: 'Descripción',
